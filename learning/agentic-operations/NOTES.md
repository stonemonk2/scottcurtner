...
- `0007-build-the-single-tool-agent.html` — guided build; tokens/cost + tool-call anatomy taught inline; logs full per-turn context (DONE). Working script: `exercises/level1_agent.py` (self-test PASSED; live path needs a key — checkpoint 0)
...
**Reference sheets:** m1/m2/m3/level-4-vocabulary.html + agent-loop-and-tool-call.html (code, built with 0007). All cross-linked.

**Blocker for the learner:** repo `.env` has NO Anthropic key (verified 2026-07-03: dotenv_values shows no ANTHROPIC_* key; ant CLI not installed). Lesson 0007 checkpoint 0 walks adding `ANTHROPIC_API_KEY` to `.env`. Until then the hands-on runs can't execute live; `level1_agent.py --selftest` works offline.
